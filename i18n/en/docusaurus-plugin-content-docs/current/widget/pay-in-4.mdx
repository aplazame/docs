...
## Integration
In case the merchant supports the product 'Pay in 4', you can add the parameter `data-pay-in-4 =' true'` to the [installment payment widget](./installment-payment) . This will cause a change in the design of the widget, which will show the necessary information only for 'Pay in 4'.
...
:::caution
  If the widget has the attribute `data-pay-in-4` but the merchant does not support the product' Pay in 4' or the amount to finance (`data-amount`) is too high, the widget it will behave exactly like the [installment payment widget](./installment-payment), as long as the merchant supports the 'Installment payment' product, inheriting all its attributes.
:::
